...
//Shadertoy
#define iGlobalTime globalTime*0.6
...
vec4 cloudColor(in vec2 fragCoord )
 {
...
void main( void )
{
	...
	vec3 result = texture2D( tPreviousPass, fragCoord).rgb;

	//fake reflection
	//result = texture(skybox,normal.xyz).rgb;

	if (length(position.xyz) < 0.000001)
	{
		//vec3 forward = normalize(vec3(viewMatrix[2][0], viewMatrix[2][1], viewMatrix[2][2]));
		//vec3 up = normalize(vec3(viewMatrix[1][0], viewMatrix[1][1], viewMatrix[1][2]));
		//vec3 right = normalize(vec3(viewMatrix[0][0], viewMatrix[0][1], viewMatrix[0][2]));
		//
		//vec2 uv = fragCoord * 2.0 - 1.0;
		//float fov = 1.;
		//
		//vec3 D = normalize(uv.x * (-right) + uv.y * up + forward * fov);
		//result = texture(skybox,D).xyz;

		result = cloudColor(fragCoord).rgb;
	}
